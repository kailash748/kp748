...
( function( $, api ) {
	...
	api.controlConstructor['abril-repeater-text'] = api.Control.extend({
...
jQuery(document).ready(function($) {
    ...
    $('#abril-img-container img').click(function() {      
        var item_id = $(this).parent().parent().parent().parent().attr('id');

        $('#' + item_id).find('li').each(function() {
            $('#' + item_id).find('img').removeClass('abril-radio-img-selected') ;
        });

        $(this).addClass('abril-radio-img-selected') ;
    });
